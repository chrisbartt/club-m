...
const AlerteCard = ({ alerte }: { alerte: Alerte }) => {
    ...
    return (
        <div
            className={`flex items-center gap-4 p-4 rounded-xl ${config.cardBg} transition-colors duration-200`}
        >
            <div
            ...
            <Button
                size="sm"
                className={`shrink-0 h-9 px-4 rounded-md shadow-none cursor-pointer text-[13px] ${alerte.actionVariant === "red"
...
const TabsAlertes = () => {
    ...
    return (
        <div className="card cardShadow w-full bg-bgCard rounded-xl overflow-hidden p-6 h-full flex flex-col">
            <Tabs defaultValue="toutes" className="flex flex-col flex-1 min-h-0">
                <TabsList
                    variant="line"
                    className="w-full justify-start h-auto p-0 bg-transparent gap-0 border-b border-colorBorder rounded-none min-h-0"
                >
                    <TabsTrigger
                        value="toutes"
