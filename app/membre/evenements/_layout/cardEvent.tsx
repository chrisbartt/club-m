    ...
    return (
        <div className="card-event bg-bgCard rounded-xl p-2">
            ...
            <div className="card-content p-3">
                ...
                <p className="text-sm text-colorMuted flex items-center gap-1 mb-3">
                    <MapPin size={16} className="text-colorMuted" />
                    <span className="text-sm text-colorMuted">{location}</span>
                </p>
                
                {/* Bloc Places */}
                {totalPlaces !== undefined && placesRestantes !== undefined && (
                    <div className="border border-dashed border-colorBorder rounded-lg p-2.5 flex items-center justify-between">
                        <span className="text-[12px] text-colorMuted">Places disponibles</span>
                        <span className="text-[13px] font-semibold text-colorTitle">
                            {placesRestantes} / {totalPlaces}
                        </span>
                    </div>
                )}
            </div>
