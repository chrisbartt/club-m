...
const DashboardHeader = () => {
  const { user, isLoading, fullName, planLabel } = useCurrentUser();
    ...
    <div className="bg-bgSidebar dark:bg-bgGray rounded-xl lg:rounded-2xl p-4 lg:p-6 mb-4 lg:mb-6 lg:mt-6 relative overflow-hidden z-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-[28px] lg:text-[36px] font-bold text-white mb-2">
            {isLoading ? (
              <span className="inline-block w-48 h-9 bg-white/20 rounded-lg animate-pulse" />
            ) : (
          ...
          <p className="text-[14px] lg:text-[16px] text-white/90 leading-relaxed">
            {isLoading ? (
              <span className="inline-block w-80 h-5 bg-white/10 rounded animate-pulse" />
            ) : (
              <>
                Tu fais partie des membres {planLabel} du Club M. Tu as accès
                à l&apos;ensemble des modules pour structurer, financer et
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
          <div className="text-[32px] lg:text-[40px] font-bold text-white mb-1">
            92%
          </div>
