...
const AlertRow = ({ type, icon, iconBgColor, title, description, secondaryInfo, button }: AlertItem) => {
    ...
    return (
        <div className={`gap-2 flex flex-col md:p-4 p-3 rounded-xl ${bgColors[type]}`}>
           <div className="flex items-start gap-4">
            ...
            {/* Button */}
            <Button
                className={`h-9 px-4 text-[13px] hidden md:flex items-center justify-center font-medium gap-2 shadow-none flex-shrink-0 ${buttonStyles[button.variant]}`}
            >
                {button.icon}
                {button.label}
            </Button>
