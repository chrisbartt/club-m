...
const MemberRow = ({ membre }: { membre: Membre }) => {
    ...
    return (
        ...
        >
            {/* Avatar */}
            <div className={`md:w-[50px] w-[40px] md:h-[50px] h-[40px] rounded-full flex items-center justify-center text-white font-semibold text-[15px] flex-shrink-0 ${getAvatarBgColor(membre.status)}`}>
                {membre.initials}
            </div>
            ...
            {/* Actions */}
            <div className="md:flex hidden items-center gap-2 flex-shrink-0">
                ...
                <Link href={`/admin/membres/liste/${membre.id}`} className="px-4 py-2 rounded-lg bg-bgSidebar dark:bg-bgGray text-white text-[13px] font-medium hover:opacity-90 transition-opacity cursor-pointer">
                    Fiche
                </Link>
            </div>
        </div>
