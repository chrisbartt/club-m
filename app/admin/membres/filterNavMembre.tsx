...
const FilterNavMembre = () => {
    ...
    const isActive = (href: string) => {
        if (href === "/admin/membres") {
            return pathname === "/admin/membres";
        }
        return pathname.startsWith(href);
    };

    return (
        <div className="filter-nav-membre flex items-center gap-2 py-[12px] bg-bgCard lg:px-4 px-3 2xl:px-6 border-t border-colorBorder sticky lg:top-[73px] top-[64px] z-50">
            <button
                type="button"
                onClick={() => scroll("left")}
                ...
                {navLinks.map((link) => {
                    ...
                    return (
                        <Link
                            key={link.href}
                            href={link.href}
                            className={`shrink-0 inline-flex items-center gap-2 md:px-5 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all duration-200 ${active
