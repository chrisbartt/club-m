...
export default function RegisterPage() {
  ...
  const [certifyWoman, setCertifyWoman] = useState(false)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!certifyWoman) {
      toast.error('Vous devez certifier etre une femme pour rejoindre Club M')
      return
    }
    if (!acceptTerms) {
  ...
  return (
    <Card>
      <CardHeader className="text-center">
        ...
        <CardDescription>Creez votre compte membre</CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="certifyWoman"
              checked={certifyWoman}
              onCheckedChange={(checked) => setCertifyWoman(checked === true)}
            />
            <Label htmlFor="certifyWoman" className="text-sm font-normal">
              Je certifie etre une femme
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox
